...
// BCKS (chốt 21/08), chỉ admin tải được.
export function downloadKetQuaKiemKeGuiMail() {
  return downloadChuDeJobBlob("/tracking/kiem-ke/ket-qua-gui-mail", "ket_qua_kiem_ke_gui_mail.xlsx");
}

// Tab "Đã kiểm" — "LCNB Thanh Lý về Kho Tổng" (24/08), 2 file riêng theo
// kho nhận, tính lại từ file log kết quả kiểm kê mỗi lần tải, chỉ admin.
// Trả về {so_dong, chua_xac_dinh} đọc từ header để báo cho admin biết ngay.
async function downloadLcnbThanhLy(kho, filename) {
  const token = getToken();
  const res = await fetch(`${API_URL}/tracking/kiem-ke/lcnb-thanh-ly/${kho}`, {
    headers: token ? { Authorization: `Bearer ${token}` } : {},
  });
  if (!res.ok) {
    const body = await res.json().catch(() => ({}));
  ...
  return { soDong, chuaXacDinh };
}

export function downloadLcnbThanhLyHni() {
  return downloadLcnbThanhLy("hni", "LCNB_Thanh_Ly_HNI.xlsx");
}

export function downloadLcnbThanhLyHcm() {
  return downloadLcnbThanhLy("hcm", "LCNB_Thanh_Ly_HCM.xlsx");
}
...
export function listChuDeJobs() {
  return apiFetch("/chu-de-jobs");
}

async function chuDeJobFormRequest(url, method, { ten_chu_de, vung, ten_shop, noi_dung_vi_pham, file }) {
  const token = getToken();
  const formData = new FormData();
...
export function claimChuDeJob(id) {
  return apiFetch(`/chu-de-jobs/${id}/claim`, { method: "POST" });
}

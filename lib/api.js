...
// /chu-de-jobs-v2 (bảng riêng, không đụng dữ liệu thật) + thêm "nhom"
// ("long_chau" | "vaccine") để phân biệt Long Châu/Vaccine. Tra cứu shop
// dùng CHUNG lookupChuDeShop() ở trên (không phụ thuộc dữ liệu job). ----------

export function listChuDeJobsV2(thang, nhom) {
...
export async function exportChuDeJobsV2(thang, nhom) {
  const token = getToken();
  const params = new URLSearchParams();
  if (thang) params.set("thang", thang);
  if (nhom) params.set("nhom", nhom);
...
async function chuDeJobV2FormRequest(url, method, { ten_chu_de, vung, ma_shop, ten_shop, noi_dung_vi_pham, nhom, file }) {
  const token = getToken();
  const formData = new FormData();
  formData.append("ten_chu_de", ten_chu_de || "");
  formData.append("vung", vung || "");
  ...
  formData.append("ten_shop", ten_shop || "");
  formData.append("noi_dung_vi_pham", noi_dung_vi_pham || "");
  formData.append("nhom", nhom || "long_chau");
  if (file) formData.append("file", file);
  const res = await fetch(`${API_URL}${url}`, {
    method,
    headers: token ? { Authorization: `Bearer ${token}` } : {},

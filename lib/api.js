...
// chạy nền -> poll -> trả về { blob, filename } (1 file kết quả tối đa 4
// sheet: tối đa 3 sheet kiểm kê đã ráp + 1 sheet Tổng hợp Kiểm Kê TTTC)
export async function tongHopBcksTttc(files) {
  ...
  while (true) {
    ...
    if (job.status === "done") {
      ...
      for (let i = 0; i < bin.length; i++) bytes[i] = bin.charCodeAt(i);
      const blob = new Blob([bytes], { type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" });
      return { blob, filename: job.result.filename || "Bao cao kiem soat TTTC.xlsx" };
    }
    if (job.status === "error") throw new Error(job.error || "Xử lý thất bại");
    if (Date.now() - startedAt > 10 * 60 * 1000) throw new Error("Quá thời gian chờ xử lý — thử lại sau");
    await new Promise((resolve) => setTimeout(resolve, 2000));

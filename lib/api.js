...
async function viPhamCaseV2FormRequest(url, method, {
  ...
  dien_giai_vi_pham, gia_tri_vi_pham, sl_so_vi_pham, trang_thai, hinh_thuc_ky_luat, gia_tri_phat, file,
}) {
  const token = getToken();
  const formData = new FormData();
...
export function downloadViPhamCaseV2File(id, filename) {
  return downloadChuDeJobBlob(`/ghi-nhan-case-v2/${id}/download`, filename || "dinh_kem");
}

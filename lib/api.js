...
// -> trả về { blob, filename, stats } (blob = 1 file .zip gồm 3 file .xlsx
// riêng từng sheet VPKM/VTYT/VX, tải 1 lần cả 3; stats = {"Kiểm Kê VPKM",
...
export async function processHoTroVx(file) {
  ...
  while (true) {
    ...
    if (job.status === "done") {
      const bin = atob(job.result.zip_base64);
      const bytes = new Uint8Array(bin.length);
      for (let i = 0; i < bin.length; i++) bytes[i] = bin.charCodeAt(i);
      const blob = new Blob([bytes], { type: "application/zip" });
      return { blob, filename: job.result.zip_filename || "Ho tro kiem ke VX.zip", stats: job.result.stats || {} };
    }
    if (job.status === "error") throw new Error(job.error || "Xử lý thất bại");
    if (Date.now() - startedAt > 10 * 60 * 1000) throw new Error("Quá thời gian chờ xử lý — thử lại sau");
    await new Promise((resolve) => setTimeout(resolve, 2000));

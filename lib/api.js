...
export function completeChuDeJob(id) {
  return apiFetch(`/chu-de-jobs/${id}/complete`, { method: "POST" });
}

export async function downloadChuDeJobFile(id, filename) {
  const token = getToken();
  const res = await fetch(`${API_URL}/chu-de-jobs/${id}/download`, {
    headers: token ? { Authorization: `Bearer ${token}` } : {},
  });
  if (!res.ok) {
    const body = await res.json().catch(() => ({}));
  ...
  a.href = url;
  a.download = filename || "data_check.xlsx";
...
// Up file tồn kho -> chạy nền -> poll -> trả về Blob file Excel kết quả để tải về
...
export function getActivityLogSummary() {
  return apiFetch("/activity-log/summary");
}

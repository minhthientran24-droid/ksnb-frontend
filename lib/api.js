...
// Tổng hợp Báo cáo Kiểm Soát Sau Kiểm Kê — bước 1: điền sheet KIEM KE từ
// file Xuất Khác - Nhập Khác (sheet THANH LY chờ rule sau).
export async function tongHopBcksFromXknk(file) {
  const token = getToken();
  const formData = new FormData();
  formData.append("file", file);
  ...
  while (true) {
    ...
    if (job.status === "done") {
      ...
      for (let i = 0; i < bin.length; i++) bytes[i] = bin.charCodeAt(i);
      const blob = new Blob([bytes], { type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" });
      return { blob, soDong: job.result.so_dong, soDongThieuGia: job.result.so_dong_thieu_gia };
    }
    if (job.status === "error") throw new Error(job.error || "Xử lý thất bại");
    if (Date.now() - startedAt > 10 * 60 * 1000) throw new Error("Quá thời gian chờ xử lý — thử lại sau");
    await new Promise((resolve) => setTimeout(resolve, 2000));

...
async function apiFetch(path, options = {}) {
  ...
  if (res.status === 401) {
    // Chỉ coi là "phiên hết hạn" khi request này ĐÃ mang theo token (tức
    // đang dùng 1 phiên có sẵn mà bị từ chối) — chốt 28/08, fix bug: nếu
    // KHÔNG có token (vd chính request đăng nhập /auth/login gõ sai mật
    // khẩu) thì đây không phải phiên hết hạn, phải trả đúng lỗi thật từ
    // backend ("Sai email hoặc mật khẩu"), không được ghi đè thành thông
    // báo gây hiểu lầm là tài khoản có phiên đăng nhập cũ bị hết hạn.
    if (token) {
      clearSession();
    ...
    throw new Error(body.detail || "Sai email hoặc mật khẩu");
  }
...
// Xoá dòng kiểm kê (chốt 28/08, chỉ admin/super_admin) — riêng dòng "Đã
// kiểm" xoá xong TỰ ĐỘNG gỡ ngược kỳ lịch làm việc về lại "Đang kiểm"
// (dùng khi gửi nhầm mail BCKS, cần gửi lại). Trả về thêm
// lichlamviec_reverted/cycle_restored để báo rõ có gỡ được kỳ không.
export function deleteKiemKeRow(id) {
  return apiFetch(`/tracking/kiem-ke/${id}`, { method: "DELETE" });
}
...
export function listMyLeaveDays() {
  return apiFetch("/leave-days/mine");
}

export function createLeaveDay(ngay_nghi, ghi_chu) {
  return apiFetch("/leave-days", { method: "POST", body: JSON.stringify({ ngay_nghi, ghi_chu }) });
}

export function deleteLeaveDay(id) {
  return apiFetch(`/leave-days/${id}`, { method: "DELETE" });
}

export function listAllLeaveDays() {
  return apiFetch("/leave-days/all");
}

export function leaveDaysByDate(date) {
  return apiFetch(`/leave-days/by-date?date=${encodeURIComponent(date)}`);
}

// ---------- Quản lý phân quyền menu ----------
export function getMenuPermissionCatalog() {
  return apiFetch("/menu-permissions/catalog");
}

export function saveMenuPermissions(matrix) {
  return apiFetch("/menu-permissions", { method: "PUT", body: JSON.stringify({ matrix }) });
}

export function getMyAllowedMenus() {
  return apiFetch("/menu-permissions/mine");
}

// Sidebar + Layout đều cần danh sách này (Sidebar để ẩn/hiện menu, Layout
// để chặn vào thẳng URL) — cache trong bộ nhớ (reset khi đăng xuất/tải lại
// trang) để KHỎI gọi API lại mỗi lần chuyển trang, tránh chậm thêm 1 lượt
// round-trip mạng mỗi lần bấm menu (đã tối ưu độ trễ mạng ở nơi khác rồi).
let _allowedMenusCache = null;
export function getMyAllowedMenusCached() {
  if (_allowedMenusCache) return Promise.resolve(_allowedMenusCache);
  return getMyAllowedMenus().then((r) => {
    _allowedMenusCache = r;
    return r;
  });
}
...
// Đính kèm THẲNG file gốc (VD .pptx) cho "Báo cáo kiểm soát theo chủ đề" —
// giữ nguyên layout gốc, không qua xử lý gì (chốt 04/09, chỉ admin/super_admin).
export async function uploadReportChuDeFile(periodLabel, file, displayName) {
  const token = getToken();
  const formData = new FormData();
  formData.append("file", file);
  if (displayName) formData.append("display_name", displayName);
  const res = await fetch(`${API_URL}/reports/${periodLabel}/chu-de-file`, {
    method: "POST",
    headers: token ? { Authorization: `Bearer ${token}` } : {},
    body: formData,
  });
  if (!res.ok) {
    const body = await res.json().catch(() => ({}));
    throw new Error(body.detail || `Lỗi upload file: ${res.status}`);
  }
  return res.json();
}

export function downloadReportChuDeFile(periodLabel, filename) {
  return downloadChuDeJobBlob(`/reports/${periodLabel}/chu-de-file`, filename || "bao_cao_chu_de.pptx");
}

export function deleteReportChuDeFile(periodLabel) {
  return apiFetch(`/reports/${periodLabel}/chu-de-file`, { method: "DELETE" });
}

// Render lại slide khi lần render lúc upload bị lỗi thoáng qua (admin)
export function retryRenderReportChuDeSlides(periodLabel) {
  return apiFetch(`/reports/${periodLabel}/chu-de-file/render-slides`, { method: "POST" });
}

// Ảnh slide cần header Authorization nên KHÔNG gán thẳng vào <img src> được
// (thẻ img không gửi kèm header) — fetch về dạng blob rồi tạo object URL.
// Nhớ URL.revokeObjectURL() khi không cần nữa (component SlideViewer tự lo).
export async function fetchReportChuDeSlideBlobUrl(periodLabel, slideNo) {
  const token = getToken();
  const res = await fetch(`${API_URL}/reports/${periodLabel}/chu-de-file/slides/${slideNo}`, {
    headers: token ? { Authorization: `Bearer ${token}` } : {},
  });
  if (!res.ok) {
    const body = await res.json().catch(() => ({}));
    throw new Error(body.detail || `Lỗi tải ảnh slide: ${res.status}`);
  }
  const blob = await res.blob();
...
// `files` (chốt 08/09) — mảng File, cho phép đính kèm NHIỀU file data
// check/job. Sửa job: files truyền vào là file MỚI, CỘNG DỒN vào danh
// sách đã có (không thay thế) — xoá 1 file lẻ dùng riêng
// deleteChuDeJobDataFile().
async function chuDeJobFormRequest(url, method, { ten_chu_de, vung, ma_shop, ten_shop, noi_dung_vi_pham, files }) {
  const token = getToken();
  const formData = new FormData();
...
// File mẫu 10 sheet cho khối "Dữ liệu tham chiếu" ở "Tải lên dữ liệu"
// (chốt 27/08 lần 24).
export function downloadReferenceFilesTemplate() {
  return downloadChuDeJobBlob("/kiem-ke-thanh-ly/reference-files/template", "Mau_Du_lieu_tham_chieu.xlsx");
}

// File mẫu "Danh sách shop" (chốt 28/08) — 2 cột bắt buộc (Mã Shop, Ngày
// mở bán) tô đỏ. Endpoint dùng JWT web chính (không qua bridge app1_).
export function downloadDanhSachShopTemplate() {
  return downloadChuDeJobBlob("/lichlamviec/v2/upload-danh-sach/template", "Mau_Danh_sach_shop.xlsx");
}
...
// `ghi_chu` (chốt 07/09) — CHỈ append vào form khi caller CÓ truyền (kể
// cả chuỗi rỗng nghĩa là chủ động xoá) — không truyền (undefined) thì bỏ
// hẳn field này, backend hiểu là "giữ nguyên ghi chú cũ" (xem Form(None)
// trong routers/chu_de_jobs.py::complete_job). Áp dụng cho đúng luồng
// "điều chỉnh lại kết quả" và luồng "Có vi phạm" (ghi-nhan-case.js) —
// cả 2 gọi hàm này KHÔNG kèm ghi_chu, không được vô tình xoá mất.
export async function completeChuDeJob(id, { ket_qua_vi_pham, ghi_chu, file }) {
  const token = getToken();
  const formData = new FormData();
  formData.append("ket_qua_vi_pham", ket_qua_vi_pham || "");
  if (ghi_chu !== undefined) formData.append("ghi_chu", ghi_chu);
...
export function downloadChuDeJobFile(jobId, fileId, filename) {
  return downloadChuDeJobBlob(`/chu-de-jobs/${jobId}/files/${fileId}/download`, filename || "data_check.xlsx");
}

export function deleteChuDeJobDataFile(jobId, fileId) {
  return apiFetch(`/chu-de-jobs/${jobId}/files/${fileId}`, { method: "DELETE" });
}
...
// ---------- Theo dõi chủ đề Ver2 (chốt 03/09) — menu thử nghiệm, chỉ
// admin/super_admin dùng được. Bản sao các hàm ở trên, trỏ sang
// /chu-de-jobs-v2 (bảng riêng, không đụng dữ liệu thật). Tra cứu shop dùng
// CHUNG lookupChuDeShop() ở trên (không phụ thuộc dữ liệu job).
// Chốt 04/09 — bỏ tham số "nhom" (Long Châu/Vaccine), view chung. ----------

export function listChuDeJobsV2(thang, tenChuDe) {
  const params = new URLSearchParams();
  if (thang) params.set("thang", thang);
  if (tenChuDe) params.set("ten_chu_de", tenChuDe);
  const qs = params.toString();
  return apiFetch(`/chu-de-jobs-v2${qs ? `?${qs}` : ""}`);
}

export function getChuDeJobV2Months() {
  return apiFetch("/chu-de-jobs-v2/months");
}

// Chốt 06/09 — combo box "Tên chủ đề" (super_admin quản lý) + thống kê số
// case theo 3 tình trạng / mỗi tên, dùng cho popup "Chọn Tên chủ đề".
export function listChuDeTopicsV2() {
  return apiFetch("/chu-de-jobs-v2/topics");
}

export function createChuDeTopicV2(tenChuDe) {
  return apiFetch("/chu-de-jobs-v2/topics", { method: "POST", body: JSON.stringify({ ten_chu_de: tenChuDe }) });
}

export function deleteChuDeTopicV2(id) {
  return apiFetch(`/chu-de-jobs-v2/topics/${id}`, { method: "DELETE" });
}

export function getChuDeTopicV2Stats(thang) {
  const qs = thang ? `?thang=${encodeURIComponent(thang)}` : "";
  return apiFetch(`/chu-de-jobs-v2/topics/stats${qs}`);
}

export async function exportChuDeJobsV2(thang, tenChuDe) {
  const token = getToken();
  const params = new URLSearchParams();
  if (thang) params.set("thang", thang);
  if (tenChuDe) params.set("ten_chu_de", tenChuDe);
  const qs = params.toString();
  const res = await fetch(`${API_URL}/chu-de-jobs-v2/export${qs ? `?${qs}` : ""}`, {
    headers: token ? { Authorization: `Bearer ${token}` } : {},
  });
  if (!res.ok) {
    const body = await res.json().catch(() => ({}));
  ...
  const filename = filenameFromContentDisposition(res, "theo_doi_chu_de_v2.xlsx");
  const blob = await res.blob();
  const objUrl = URL.createObjectURL(blob);
  const a = document.createElement("a");
  a.href = objUrl;
...
async function chuDeJobV2FormRequest(url, method, { ten_chu_de, loai_vi_pham, vung, ma_shop, ten_shop, noi_dung_vi_pham, file }) {
  const token = getToken();
  const formData = new FormData();
  formData.append("ten_chu_de", ten_chu_de || "");
  formData.append("loai_vi_pham", loai_vi_pham || "");
  formData.append("vung", vung || "");
  formData.append("ma_shop", ma_shop || "");
  formData.append("ten_shop", ten_shop || "");
  formData.append("noi_dung_vi_pham", noi_dung_vi_pham || "");
...
export function createChuDeJobV2(data) {
  return chuDeJobV2FormRequest("/chu-de-jobs-v2", "POST", data);
}

export function updateChuDeJobV2(id, data) {
  return chuDeJobV2FormRequest(`/chu-de-jobs-v2/${id}`, "PUT", data);
}

export function deleteChuDeJobV2(id) {
  return apiFetch(`/chu-de-jobs-v2/${id}`, { method: "DELETE" });
}

export async function bulkUploadChuDeJobsV2(file) {
  const token = getToken();
  const formData = new FormData();
  formData.append("file", file);
  const res = await fetch(`${API_URL}/chu-de-jobs-v2/bulk-upload`, {
    method: "POST",
    headers: token ? { Authorization: `Bearer ${token}` } : {},
    body: formData,
  });
...
export function downloadChuDeJobV2BulkUploadTemplate() {
  return downloadChuDeJobBlob("/chu-de-jobs-v2/bulk-upload/template", "Mau_dang_chu_de_hang_loat_Ver2.xlsx");
}

export function claimChuDeJobV2(id, supporterUserIds = []) {
  return apiFetch(`/chu-de-jobs-v2/${id}/claim`, {
    method: "POST",
    body: JSON.stringify({ supporter_user_ids: supporterUserIds }),
  });
}

export function unclaimChuDeJobV2(id) {
  return apiFetch(`/chu-de-jobs-v2/${id}/unclaim`, { method: "POST" });
}

export function addChuDeJobV2Supporters(id, supporterUserIds) {
  return apiFetch(`/chu-de-jobs-v2/${id}/add-supporters`, {
    method: "POST",
    body: JSON.stringify({ supporter_user_ids: supporterUserIds }),
  });
}

export function listKsnbForChuDeV2() {
  return apiFetch("/chu-de-jobs-v2/ksnb-list");
}

// Chốt 23/09 — thêm ghi_chu (khớp bản gốc completeChuDeJob ở trên): chỉ
// append khi caller CÓ truyền (kể cả chuỗi rỗng = chủ động xoá), không
// truyền thì bỏ hẳn field, backend hiểu là giữ nguyên ghi chú cũ.
export async function completeChuDeJobV2(id, { ket_qua_vi_pham, ghi_chu, file }) {
  const token = getToken();
  const formData = new FormData();
  formData.append("ket_qua_vi_pham", ket_qua_vi_pham || "");
  if (ghi_chu !== undefined) formData.append("ghi_chu", ghi_chu);
  if (file) formData.append("file", file);
  const res = await fetch(`${API_URL}/chu-de-jobs-v2/${id}/complete`, {
    method: "POST",
    headers: token ? { Authorization: `Bearer ${token}` } : {},
    body: formData,
  });
...
export function downloadChuDeJobV2File(id, filename) {
  return downloadChuDeJobBlob(`/chu-de-jobs-v2/${id}/download`, filename || "data_check.xlsx");
}

export function downloadChuDeJobV2ResultFile(id, filename) {
  return downloadChuDeJobBlob(`/chu-de-jobs-v2/${id}/download-result`, filename || "ket_qua.xlsx");
}

// ---------- Ghi nhận case vi phạm Ver2 (chốt 06/09) — menu thử nghiệm,
// chỉ admin/super_admin dùng được. Bảng riêng vi_pham_cases_v2, ĐỘC LẬP
// với "Theo dõi chủ đề Ver2" ở trên dù cùng nhóm menu thử nghiệm. ----------

export function listViPhamCasesV2() {
  return apiFetch("/ghi-nhan-case-v2");
}

// `files` (chốt 06/09 lần 6) — mảng File, cho phép đính kèm NHIỀU file/case.
// Sửa case: files truyền vào là file MỚI, CỘNG DỒN vào danh sách đã có
// (không thay thế) — xoá 1 file lẻ dùng riêng deleteViPhamCaseV2File().
async function viPhamCaseV2FormRequest(url, method, {
  chu_de_vi_pham, loai_vi_pham, ma_shop, ten_shop, vung, nhan_vien_vi_pham,
  dien_giai_vi_pham, gia_tri_vi_pham, sl_so_vi_pham, trang_thai, hinh_thuc_ky_luat, gia_tri_phat, files,
}) {
  const token = getToken();
  const formData = new FormData();
  formData.append("chu_de_vi_pham", chu_de_vi_pham || "");
  formData.append("loai_vi_pham", loai_vi_pham || "");
  formData.append("ma_shop", ma_shop || "");
  formData.append("ten_shop", ten_shop || "");
  formData.append("vung", vung || "");
  formData.append("nhan_vien_vi_pham", nhan_vien_vi_pham || "");
  formData.append("dien_giai_vi_pham", dien_giai_vi_pham || "");
  formData.append("gia_tri_vi_pham", gia_tri_vi_pham || "");
  formData.append("sl_so_vi_pham", sl_so_vi_pham || "");
  formData.append("trang_thai", trang_thai || "dang_xu_ly");
  formData.append("hinh_thuc_ky_luat", hinh_thuc_ky_luat || "");
  formData.append("gia_tri_phat", gia_tri_phat || "");
  (files || []).forEach((f) => formData.append("files", f));
  const res = await fetch(`${API_URL}${url}`, {
    method,
    headers: token ? { Authorization: `Bearer ${token}` } : {},
...
export function createViPhamCaseV2(data) {
  return viPhamCaseV2FormRequest("/ghi-nhan-case-v2", "POST", data);
}

export function updateViPhamCaseV2(id, data) {
  return viPhamCaseV2FormRequest(`/ghi-nhan-case-v2/${id}`, "PUT", data);
}

export function deleteViPhamCaseV2(id) {
  return apiFetch(`/ghi-nhan-case-v2/${id}`, { method: "DELETE" });
}

export function downloadViPhamCaseV2File(caseId, fileId, filename) {
  return downloadChuDeJobBlob(`/ghi-nhan-case-v2/${caseId}/files/${fileId}/download`, filename || "dinh_kem");
}

export function deleteViPhamCaseV2File(caseId, fileId) {
  return apiFetch(`/ghi-nhan-case-v2/${caseId}/files/${fileId}`, { method: "DELETE" });
}
...
// ---------- Dữ liệu Lũy Kế (chốt 27/08, thêm chiều tháng lần 2) — mỗi
// tháng 1 bộ data riêng, up tháng nào chỉ xóa/ghi đúng tháng đó ----------
export function getLuyKeStatus() {
  return apiFetch("/luy-ke/status");
}

export async function uploadLuyKe(thang, file) {
  const token = getToken();
  const formData = new FormData();
  formData.append("thang", thang);
  formData.append("file", file);
  const res = await fetch(`${API_URL}/luy-ke/upload`, {
    method: "POST",
    headers: token ? { Authorization: `Bearer ${token}` } : {},
    body: formData,
  });
...
// `thang` ("YYYY-MM") tùy chọn — không truyền thì lấy tháng có data gần
// nhất (chốt 27/08 lần 5, thêm chiều tháng — mỗi tháng 1 bộ data riêng).
export function getXknkCanTon(thang) {
  return apiFetch(`/xknk/can-ton${thang ? `?thang=${encodeURIComponent(thang)}` : ""}`);
}

export function getXknkCanTonMonths() {
  return apiFetch("/xknk/can-ton/months");
}
...
export async function uploadXknkCanTon(thang, file) {
  const token = getToken();
  const formData = new FormData();
  formData.append("thang", thang);
...
// ---------- Tự khai báo "KSNB kiểm kê trực tiếp" (03/09) — mọi user đăng
// nhập tự khai ở "Lịch làm việc & nghỉ phép", đẩy thẳng vào cùng bảng với
// "Đề xuất kiểm kê" -> "Đề xuất KSNB kiểm kê trực tiếp" (khác endpoint
// admin ở trên — KHÔNG cần quyền Admin/Editor). ----------
export function listMyDeXuatKsnb() {
  return apiFetch("/de-xuat-kiem-ke/ksnb/self");
}
export function upsertMyDeXuatKsnb(payload) {
  return apiFetch("/de-xuat-kiem-ke/ksnb/self", { method: "POST", body: JSON.stringify(payload) });
}
export function deleteMyDeXuatKsnb(id) {
  return apiFetch(`/de-xuat-kiem-ke/ksnb/self/${id}`, { method: "DELETE" });
}

// ---------- "Hỗ trợ shop" > tab "Hỗ trợ check lệch tồn VX" (chốt 09/09).
// Nhóm "quản lý phiếu" — cần đăng nhập (menu dành cho mọi role). Nhóm
// "public" — KHÔNG cần đăng nhập (dùng bởi trang công khai
// pages/kiem-tra-ton/[token].js, apiFetch tự bỏ qua header Authorization
// khi không có token trong localStorage nên tái dùng được luôn). ----------
// Chốt 09/09 lần 4 — BỎ tạo phiếu bằng tay (dán text qua form), CHỈ còn
// tạo phiếu qua import Excel (importCheckLechTonVxExcel bên dưới).
export function downloadCheckLechTonVxTemplate() {
  return downloadChuDeJobBlob("/check-lech-ton-vx/phieu/template", "Mau_check_lech_ton_VX.xlsx");
}
// Import phiếu từ file Excel (VD "Barcode <mã shop>.xlsx", cột Mã Shop/Tên
// Shop/Tên SP/Code) — trả về MẢNG phiếu (file gộp nhiều shop thì tạo
// nhiều phiếu cùng lúc). multipart/form-data nên KHÔNG dùng apiFetch
// (set sẵn Content-Type: application/json) — cùng pattern uploadAvatar.
export async function importCheckLechTonVxExcel(file) {
  const token = getToken();
  const formData = new FormData();
  formData.append("file", file);
  const res = await fetch(`${API_URL}/check-lech-ton-vx/phieu/import-excel`, {
    method: "POST",
    headers: token ? { Authorization: `Bearer ${token}` } : {},
    body: formData,
  });
...
export function listCheckLechTonVxPhieu() {
  return apiFetch("/check-lech-ton-vx/phieu");
}
export function getCheckLechTonVxPhieu(id) {
  return apiFetch(`/check-lech-ton-vx/phieu/${id}`);
}
export function reopenCheckLechTonVxPhieu(id) {
  return apiFetch(`/check-lech-ton-vx/phieu/${id}/mo-lai`, { method: "POST" });
}
export function deleteCheckLechTonVxPhieu(id) {
  return apiFetch(`/check-lech-ton-vx/phieu/${id}`, { method: "DELETE" });
}

export function getCheckLechTonVxPublic(token) {
  return apiFetch(`/check-lech-ton-vx/public/${token}`);
}
export function scanCheckLechTonVxPublic(token, maQuet) {
  return apiFetch(`/check-lech-ton-vx/public/${token}/scan`, {
    method: "POST",
    body: JSON.stringify({ ma_quet: maQuet }),
  });
}
export function hoanTatCheckLechTonVxPublic(token) {
  return apiFetch(`/check-lech-ton-vx/public/${token}/hoan-tat`, { method: "POST" });
}
// Cho phép NV tự xoá 1 lượt quét bị bắn sai (chốt 09/09 lần 7) — trả về
// phiếu đã cập nhật (items/khong_ton) để cập nhật lại UI ngay.
export function deleteCheckLechTonVxPublicScan(token, scanLogId) {
  return apiFetch(`/check-lech-ton-vx/public/${token}/scan/${scanLogId}`, { method: "DELETE" });
}
